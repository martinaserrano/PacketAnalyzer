...
#  prn: function to apply to each packet. If something is returned, it is displayed; in this case is returned def process_packet(which contains 3 variables: Ip Source, Ip Destination and packetInfo which stores information of sniffed packets).
...
# Normally, echo request have an ICMPType == 8 and the Code must be 0, in other case the code is invalid and we need to check the type of the next packets from the same ip
# So the code does a second check: If at the same ip corrispond another ICMPtype == 13 or IcmpType == 15 or IcmpType ==17 (which idenfity ICMP get address mask, ICMP get timestamp, ICMP get information)
